<template>
<div v-if="props.isOpen" class="fixed inset-0 backdrop-blur-3xl bg-black bg-opacity-60 flex items-center justify-center p-4 sm:p-8">
	  <div class="relative bg-white p-4 sm:p-6 rounded shadow-lg w-full sm:w-1/2 max-w-4xl mx-auto">
		...
		<div class="container p-2 mx-auto rounded-md sm:p-4 dark:text-gray-800 dark:bg-gray-50">
		  ...
		  <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 items-center">
			<img v-if="props.image" :src="props.image" alt="Selected" class="h-20 sm:h-8 mb-4 rounded-md" />
			<div class="w-px h-12 bg-gray-300 hidden sm:block"></div>
			<div class="text-center sm:text-left">
			  <h3 v-if="props.sede">{{ props.sede }}</h3>
			  <h3 v-if="props.departamento">({{ props.departamento }})</h3>
			</div>
		  </div>
			...
			<table class="min-w-full text-xs">
			  ...
			  <tbody>
				<tr
				  ...
				  <td class="py-2">{{ item.HorarioAtencion }}</td>
